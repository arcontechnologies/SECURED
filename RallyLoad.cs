...
namespace Rally
{
    ...
    class RallyLoad
    {
                ...
                try
                {
                    if (is_stagging == true)
                    {
                        foreach (var t in listtable)
                        {
                            ...
                            cmd.Parameters.Add(new SqlParameter("@Tablename", Helpers.SecurityValidation.GetSanitizedTableName(t.ToString())));
                            cmd.ExecuteNonQuery();
                        }
                    }
                }
                    ...
                    for (int count = 0; count <= dataset.Tables.Count - 1; count++)
                    {
                        ...
                        if (listtable.Contains(dataset.Tables[count].TableName.ToString()))
                        {
                            ...
                            if (i == 0)
                            {
                                ...
                                bulkcopy.DestinationTableName = Helpers.SecurityValidation.GetSanitizedTableName(rootnode.ToUpper());
                            }
                            else
                            {
                                ResultDataTableMapping = Helpers.CompareRows(Helpers.GetSQLTable(dbserver, database, Helpers.SecurityValidation.GetSanitizedTableName(rootnode.ToUpper())), Helpers.GetAGTable(inputDataTableMapping), inputDataTableMapping, listmappedcolumn);
                                bulkcopy.DestinationTableName = Helpers.SecurityValidation.GetSanitizedTableName(rootnode.ToUpper());
                            }
        ...
        static void UpdateDatatable()
        {
            ...
            using (SqlConnection connection = new SqlConnection(connString))
            {
                string query = Helpers.SecurityValidation.GetSanitizedTableName("RevisionHistory");
            ...
            using (SqlConnection connection = new SqlConnection(connString))
            {
                ...
                cmd.Parameters.Clear();
                cmd.Parameters.Add(new SqlParameter("@Tablename", Helpers.SecurityValidation.GetSanitizedTableName("RevisionHistory")));
                cmd.ExecuteNonQuery();


                SqlBulkCopy bulkcopy = new SqlBulkCopy(conn)
                {
                    DestinationTableName = Helpers.SecurityValidation.GetSanitizedTableName("RevisionHistory")
                };
        ...
        static void GetMilestones(string table, string PortfolioItemType)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connString))
            {
                string query = "SELECT M._ref as url, P.ObjectUUID as PortfolioItemID FROM " + Helpers.SecurityValidation.GetSanitizedTableName(table.ToUpper());
        ...
        static void GetRevisionHistory(string table)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connString))
            {
                string query = Helpers.SecurityValidation.GetSanitizedTableName(table.ToUpper());
                SqlCommand cmd = new SqlCommand(query, conn);
                conn.Open();
                SqlDataAdapter da = new SqlDataAdapter(cmd);
                da.Fill(opus);
            ...
            using (SqlConnection connection = new SqlConnection(connString))
            {
                ...
                SqlBulkCopy bulkcopy = new SqlBulkCopy(conn)
                {
                    DestinationTableName = Helpers.SecurityValidation.GetSanitizedTableName("Revision")
                };
        ...
        public static void LoadFeatureException(string dbserver, string database, string table, DataTable ConfigTable)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connString))
            {
                ...
                cmd.Parameters.Clear();
                cmd.Parameters.Add(new SqlParameter("@Tablename", Helpers.SecurityValidation.GetSanitizedTableName("Expertise")));
                cmd.ExecuteNonQuery();

                // // truncate ExpertiseDemands table
                SqlBulkCopy bulkcopy = new SqlBulkCopy(conn)
                {
                    DestinationTableName = Helpers.SecurityValidation.GetSanitizedTableName(table.ToUpper())
                };
                ...
                {
                    Connection = conn,
                    CommandText = Helpers.SecurityValidation.GetSanitizedTableName("query"),
        ...
        static void LoadDatasetExceptionUserstory(DataSet dataset, DataTable ConfigTable, string table, string table_count)
        {
            ...
            finally
            {
                ...
                using (SqlConnection connection = new SqlConnection(connString))
                {
                    ...
                    SqlBulkCopy bulkcopy = new SqlBulkCopy(conn)
                    {
                        DestinationTableName = Helpers.SecurityValidation.GetSanitizedTableName("USERSTORY")
                    };
        ...
        static void MakelinksforAffectedItemsforRisk(string dbserver, string database)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connString))
            {
                ...
                SqlBulkCopy bulkcopy = new SqlBulkCopy(conn)
                {
                    DestinationTableName = Helpers.SecurityValidation.GetSanitizedTableName("LINKS")
                };
